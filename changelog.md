...
- **Multi-Row Split Spectrogram View**: Added support for splitting the main Spectrogram into multiple vertically-stacked periodic row plots to analyze pulsed, bursty, or frequency-hopping signals. The feature is controlled from a new tabbed layout in the left sidebar where the "Number of Rows" checkbox acts as the split view activator. Individual row plots display absolute time ranges on their X-axes, while their zoom ranges are synchronized relatively (zooming one row pans and zooms all rows proportionally). Features include intelligent unit-aware focus formatting for frequency inputs, auto-calculation of row segments, and live brightness/contrast sync across all rows.
